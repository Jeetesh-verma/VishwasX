...
import { BookOpen, Zap, Shield, CheckCircle, ArrowRight, PlayCircle, Clock, Star, Trophy, Sparkles, RefreshCcw } from "lucide-react";
...
const LearningRoadmapPage = () => {
  ...
  useEffect(() => {
    // 1. Retrieve AI feedback from localStorage
    const cached = localStorage.getItem("vishwasx_ai_feedback");
    
    if (cached) {
      try {
        const aiData = JSON.parse(cached);
        const gaps = aiData?.role_analysis?.missing_skills || [];
        
        if (gaps.length > 0) {
          setMissingSkills(gaps);
          setIsPersonalized(true);
          
          // 2. Filter resources based on missing skills
          const matchedResources: LearningResource[] = [];
          const seenIds = new Set<string>();

          // Prioritize courses that match the missing skills
          gaps.forEach((skill: string) => {
            const matches = learningResources.filter(r => 
              r.skillsCovered.some(s => s.toLowerCase().includes(skill.toLowerCase()))
            );
            
            matches.forEach(m => {
              if (!seenIds.has(m.id)) {
                matchedResources.push(m);
                seenIds.add(m.id);
              }
            });
          });

          // Limit to a manageable "path" (e.g., 5-6 steps)
          setResources(matchedResources.slice(0, 6));
        } else {
          // If no gaps, show some general "Top Resources"
    ...
    <div className="min-h-screen px-0 py-20 bg-background flex flex-col items-center overflow-x-hidden">
      ...
      >
        ...
        <p className="text-xl text-muted-foreground font-medium max-w-xl mx-auto leading-relaxed">
          {isPersonalized 
            ? `We've synced with your AI Career Profile to bridge your specific ${missingSkills.length} skill gaps.`
        ...
        <div className="space-y-32 relative">
          {resources.map((resource, idx) => (
            ...
            >
              ...
              <div className={`hidden md:block absolute top-1/2 h-[2px] w-12 bg-primary/20 -translate-y-1/2 z-10 ${idx % 2 === 0 ? 'left-[calc(50%+24px)]' : 'right-[calc(50%+24px)]'}`} />
              ...
              >
                <div className="group relative">
                  ...
                  <div className="relative bg-card/60 backdrop-blur-xl rounded-3xl p-8 border border-border/50 shadow-sm hover:shadow-2xl hover:border-primary/30 transition-all duration-500 group-hover:-translate-y-2">
                    ...
                    <p className="text-muted-foreground mb-8 text-sm leading-relaxed line-clamp-2">
                      {resource.description || "Master these essential skills with expert-led instruction and high-impact hands-on projects designed for immediate career application."}
                    </p>

                    <div className="flex flex-wrap gap-2 mb-8">
                      {resource.skillsCovered.map(skill => (
                        <span key={skill} className="px-3 py-1.5 rounded-xl bg-secondary/30 text-[10px] font-bold text-secondary-foreground uppercase tracking-tight">
                          {skill}
                        </span>
                      ))}
                    </div>
                    ...
                    >
                      Initialize Module <ArrowRight className="h-4 w-4 group-hover/btn:translate-x-1 transition-transform" />
                    </a>
                  </div>
                </div>
              </motion.div>
          ...
          {resources.length > 0 && (
            ...
            >
              ...
              <div className="mt-10 text-center space-y-2">
                <h3 className="text-3xl font-black tracking-tight italic">Destiny Awaits</h3>
                <p className="text-muted-foreground font-medium">Your skills are now ready for the global stage.</p>
              </div>
            </motion.div>
          )}

          {/* Empty State / No Knowledge Gaps */}
          {(resources.length === 0 || !isPersonalized) && !isPersonalized && (
            <div className="text-center py-20 bg-card/40 backdrop-blur-md rounded-[3rem] border border-dashed border-border p-12 max-w-2xl mx-auto shadow-2xl">
              ...
              <p className="text-muted-foreground text-lg px-8 mb-10">Upload your resume on the dashboard for a secure AI analysis. We'll build a personalized path based on your unique skill gaps.</p>
              <button 
          ...
          {isPersonalized && resources.length === 0 && (
             <div className="text-center py-24 bg-card/40 backdrop-blur-md rounded-[3rem] border border-dashed border-border p-12 max-w-2xl mx-auto shadow-2xl">
                ...
                <h2 className="text-3xl font-black mb-4">Elite Readiness Achieved</h2>
                <p className="text-muted-foreground text-lg px-8">Our AI has analyzed your profile and determined you are currently exceeding market standards. No major skill gaps detected!</p>
             </div>
          )}
        </div>
      </div>
